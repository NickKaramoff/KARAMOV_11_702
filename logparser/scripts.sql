...
SELECT count(*)
FROM
  (SELECT DISTINCT client_id FROM entries WHERE url LIKE '%order%') as orders;

-- 4. Сколько в среднем заказов оформляется за день?
SELECT avg(orders)::NUMERIC(10,3)
FROM
  (SELECT date_time::DATE, count(url) as orders FROM entries WHERE url LIKE '%order%' GROUP BY date_time::DATE) as order_count;
...
FROM
  (SELECT client_id, date_time as order_t FROM entries WHERE url='/order.phtml') as order_t_t
  JOIN
...
SELECT avg(orders)::NUMERIC(10,3)
FROM (SELECT date_trunc('hour', date_time), count(url) as orders FROM entries GROUP BY date_trunc('hour', date_time)) as view_count;
